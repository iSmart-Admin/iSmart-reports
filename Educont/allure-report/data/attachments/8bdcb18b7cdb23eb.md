# Page snapshot

```yaml
- generic [ref=e8]:
  - generic:
    - generic [ref=e9]:
      - generic [ref=e10]:
        - generic [ref=e11]: Меню разработчика
        - button [ref=e13] [cursor=pointer]:
          - img [ref=e15] [cursor=pointer]
      - button "Выйти из тренажера" [ref=e17] [cursor=pointer]
      - button "Показывать тест школы в своб. регистрации" [ref=e18] [cursor=pointer]:
        - text: Показывать тест школы
        - text: в своб. регистрации
      - button "Выйти из школьного учителя" [ref=e19] [cursor=pointer]
    - generic [ref=e23]:
      - generic [ref=e24]:
        - region "Задача для решения" [active] [ref=e26] [cursor=pointer]:
          - generic [ref=e27] [cursor=pointer]:
            - generic [ref=e29]:
              - button "Вернуться обратно" [ref=e30] [cursor=pointer]:
                - img [ref=e33] [cursor=pointer]
                - text: Вернуться обратно
              - generic [ref=e35]:
                - generic [ref=e36]:
                  - button "1" [ref=e39] [cursor=pointer]
                  - button "2" [ref=e42] [cursor=pointer]
                  - button "3" [ref=e45] [cursor=pointer]
                  - button "4" [ref=e48] [cursor=pointer]
                  - button "5" [ref=e51] [cursor=pointer]
                  - button "6" [ref=e54] [cursor=pointer]
                  - button "7" [ref=e57] [cursor=pointer]
                  - button "8" [ref=e60] [cursor=pointer]
                  - button "9" [ref=e63] [cursor=pointer]
                  - button "10" [ref=e66] [cursor=pointer]
                - generic [ref=e67]:
                  - button [disabled] [ref=e68]:
                    - img [ref=e70]
                  - button [ref=e73] [cursor=pointer]:
                    - img [ref=e75] [cursor=pointer]
            - generic [ref=e78]:
              - generic [ref=e80]:
                - img [ref=e84]
                - heading "Выбери и укажи строку, в которой записано сложноподчинённое предложение." [level=4] [ref=e86]:
                  - generic [ref=e88]: Выбери и укажи строку, в которой записано сложноподчинённое предложение.
              - img "decore" [ref=e93]
              - generic [ref=e96]:
                - generic [ref=e98] [cursor=pointer]:
                  - generic [ref=e102] [cursor=pointer]: Было так тихо, как только бывает в лесу в безветренный день.
                  - emphasis [ref=e107] [cursor=pointer]: (А.И. Куприн)
                - generic [ref=e109] [cursor=pointer]:
                  - generic [ref=e113] [cursor=pointer]: Девушка бросает на копну деревянные грабли.
                  - emphasis [ref=e118] [cursor=pointer]: (В. Песков)
                - generic [ref=e120] [cursor=pointer]:
                  - generic [ref=e124] [cursor=pointer]: Липовый лёгкий лист разметало по улицам и тротуарам, а тополёвый лежал кругами возле деревьев, серея шершавой изнанкой.
                  - emphasis [ref=e129] [cursor=pointer]: (В.П. Астафьев)
        - generic [ref=e134]:
          - generic [ref=e137]:
            - button [disabled] [ref=e139]:
              - img [ref=e141]
            - generic [ref=e144]: 100%
            - button [ref=e146] [cursor=pointer]:
              - img [ref=e148] [cursor=pointer]
          - button "2 Показать подсказку" [ref=e153] [cursor=pointer]:
            - generic [ref=e156] [cursor=pointer]: "2"
            - generic [ref=e157] [cursor=pointer]: Показать подсказку
      - generic [ref=e161]:
        - generic [ref=e164]: Интеллектуальный тренажер
        - button "Управление учениками" [ref=e170] [cursor=pointer]:
          - img [ref=e173] [cursor=pointer]
          - text: Управление учениками
        - generic [ref=e175]:
          - generic [ref=e177]:
            - button "Черновик" [ref=e178] [cursor=pointer]:
              - img [ref=e181] [cursor=pointer]
              - text: Черновик
            - button "Сообщить об ошибке" [ref=e184] [cursor=pointer]:
              - img [ref=e187] [cursor=pointer]
              - text: Сообщить об ошибке
            - generic [ref=e189]:
              - button "Часто задаваемые вопросы" [ref=e191] [cursor=pointer]:
                - img [ref=e194] [cursor=pointer]
                - text: Часто задаваемые вопросы
              - generic [ref=e196]:
                - button "Информация об уроке" [ref=e198] [cursor=pointer]:
                  - img [ref=e201] [cursor=pointer]
                  - text: Информация об уроке
                - generic [ref=e203]: Вы привязаны к школе Тест educont 2
          - generic [ref=e204]:
            - generic [ref=e205]:
              - img [ref=e207]
              - generic [ref=e209]: Реши задание, прежде чем подтвердить
            - button "Далее" [ref=e210] [cursor=pointer]:
              - text: Далее
              - img [ref=e213] [cursor=pointer]
```